...
__attribute__((constructor(101))) void envs_init(void) {
    ...
    envs.IAXL_ZIP_JOB_SIZE = env_int("IAXL_ZIP_JOB_SIZE", 0);
    ...
    envs.IAXL_IAA_ZIP_PATH = env_str("IAXL_IAA_ZIP_PATH", "hardware");
    ...
    if (!printed) {
        ...
        if (cpus < envs.IAXL_OMP_THREAD_NUM) {
         ...
         printf("[iaxl] config: qat_zip=%s iaa_zip=%s cpu_zip=%s qat_instances=%d "
             "iaa_instances=%d cpu_zip_threads=%d zip_job_size=%d "
             "omp_threads=%d cpus=%d "
               ...
               envs.IAXL_CPU_ZIP_THREADS, envs.IAXL_ZIP_JOB_SIZE,
