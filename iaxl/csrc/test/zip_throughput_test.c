...
// Measures raw compression and decompression throughput of the QAT and IAA backends, alone
// and combined, using the same shared-task-pool worker loop as kv_zip but without the
// torch/kv glue. The --pack flag adds kv_zip's per-chunk malloc+memcpy so its cost can be
// isolated; --mode selects compress, decompress, or both.
...
static int submit_inflate(Backend b, int slot, void *src, int len) {
...
struct Config {
    ...
    size_t nchunk;
    unsigned char *src;
    // Per-backend compressed copy of every chunk: IAA cannot decode QAT's 32 KB history
    // window, so each backend has to inflate the stream it produced itself.
    unsigned char **zbuf[2];
    int *zlen[2];
};

static int submit_task(const struct Config *c, Backend b, int slot, size_t i) {
    if (c->decompress)
        return submit_inflate(b, slot, c->zbuf[b][i], c->zlen[b][i]);
    return submit(b, slot, c->src + i * c->chunk, (int)c->chunk);
}

static int build_streams(struct Config *c, Backend b) {
    unsigned char *pool = malloc(c->nchunk * c->chunk);
    c->zbuf[b] = malloc(c->nchunk * sizeof(*c->zbuf[b]));
    c->zlen[b] = malloc(c->nchunk * sizeof(*c->zlen[b]));
    if (!pool || !c->zbuf[b] || !c->zlen[b])
        return -1;

    size_t off = 0;
    for (size_t i = 0; i < c->nchunk; i++) {
        void *out;
        int out_len;
        if (submit(b, 0, c->src + i * c->chunk, (int)c->chunk) != 0)
            return -1;
        if (wait_one(b, 0, &out, &out_len) != 0 || out_len <= 0)
            return -1;
        memcpy(pool + off, out, (size_t)out_len);
        c->zbuf[b][i] = pool + off;
        c->zlen[b][i] = out_len;
        off += (size_t)out_len;
    }

    // Round-trip one chunk so the decompress pass is known to be doing real work.
    void *out;
    int out_len;
    if (submit_inflate(b, 0, c->zbuf[b][0], c->zlen[b][0]) != 0)
        return -1;
    if (wait_one(b, 0, &out, &out_len) != 0 || (size_t)out_len != c->chunk ||
        memcmp(out, c->src, c->chunk) != 0)
        return -1;
    return 0;
}

static double run_pass(const struct Config *c, size_t *in_bytes, size_t *out_bytes) {
...
#pragma omp parallel num_threads(workers)
    {
        ...
        for (int k = 0; k < depth; k++) {
            ...
            if (i >= c->nchunk)
                break;
            if (submit_task(c, backend, base + k, i) != 0)
                abort();
            local_in += c->decompress ? (size_t)c->zlen[backend][i] : c->chunk;
            active++;
        }
        ...
        for (int s = 0; in_flight > 0; s = (s + 1) % active) {
            ...
            if (i < c->nchunk) {
                if (submit_task(c, backend, base + s, i) != 0)
                    abort();
                local_in += c->decompress ? (size_t)c->zlen[backend][i] : c->chunk;
            } else {
                draining = 1;
        ...
        atomic_fetch_add(&total_in, local_in);
        atomic_fetch_add(&total_out, local_out);
    }
    const double dt = now_sec() - t0;
    *in_bytes = atomic_load(&total_in);
    *out_bytes = atomic_load(&total_out);
    return dt;
}
...
static void report(const char *label, const struct Config *c, int reps) {
    ...
    for (int r = 0; r < reps; r++) {
        const double dt = run_pass(c, &in_bytes, &out_bytes);
        sum += dt;
        if (dt < best)
            best = dt;
    }
    // Both directions are rated on uncompressed bytes so the numbers stay comparable.
    const double bytes = (double)c->nchunk * (double)c->chunk;
    const double ratio = c->decompress ? (double)out_bytes / (double)in_bytes
                                       : (double)in_bytes / (double)out_bytes;
    printf("%-28s workers=%-3d best %6.2f GB/s   avg %6.2f GB/s   ratio %.3fx\n", label,
           c->qat_inst + c->iaa_inst, bytes / best / 1e9, bytes / (sum / reps) / 1e9, ratio);
    fflush(stdout);
}
...
int main(int argc, char **argv) {
    ...
    static struct option opts[] = {{"qat-per-dev", 1, 0, 'q'}, {"iaa-per-dev", 1, 0, 'a'},
                                   ...
                                   {"mode", 1, 0, 'M'},        {0, 0, 0, 0}};
    int o;
    while ((o = getopt_long(argc, argv, "q:a:Q:A:d:c:m:r:M:", opts, NULL)) != -1) {
        switch (o) {
    ...
    const int do_compress = strcmp(mode, "decompress") != 0;
    const int do_decompress = strcmp(mode, "compress") != 0;
    if (strcmp(mode, "compress") != 0 && strcmp(mode, "decompress") != 0 &&
        strcmp(mode, "both") != 0) {
        fprintf(stderr, "[throughput] --mode must be compress, decompress or both\n");
        return 2;
    }
    ...
    printf("[throughput] qat_instances=%d iaa_instances=%d depth=%d/%d chunk=%zu B data=%d MiB "
           "reps=%d mode=%s\n\n",
           qat_inst, iaa_inst, c.qat_depth, c.iaa_depth, chunk, mib, reps, mode);

    if (do_decompress &&
        (build_streams(&c, BACKEND_QAT) != 0 || build_streams(&c, BACKEND_IAA) != 0)) {
        fprintf(stderr, "[throughput] preparing compressed streams failed\n");
        return 1;
    }

    for (int dec = 0; dec <= 1; dec++) {
        if (dec ? !do_decompress : !do_compress)
            continue;
        c.decompress = dec;
        for (int pack = 0; pack <= 1; pack++) {
            c.pack = pack;
            printf("=== %s, %s ===\n", dec ? "decompress" : "compress",
