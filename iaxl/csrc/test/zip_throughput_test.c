...
int main(int argc, char **argv) {
    int qat_per_dev = 2, iaa_per_node = 4, depth = 4, reps = 5, mib = 512;
    size_t chunk = 32768;
    const char *qat_dev = "0,1,2,3,4,5";
    const char *iaa_dev = "auto";

    static struct option opts[] = {{"qat-per-dev", 1, 0, 'q'}, {"iaa-per-node", 1, 0, 'a'},
    ...
    while ((o = getopt_long(argc, argv, "q:a:Q:A:d:c:m:r:", opts, NULL)) != -1) {
        ...
        case 'a': iaa_per_node = atoi(optarg); break;
    ...
    snprintf(buf, sizeof(buf), "%d", iaa_per_node);
