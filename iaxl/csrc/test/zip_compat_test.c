...
int main(int argc, char **argv) {
    ...
    static const int IAA_HISTORY_LIMIT = 4096;
    const char *devices = argc > 1 ? argv[1] : NULL;
    int status = 1;
    int failures = 0;
    unsigned char *input = NULL;
    unsigned char *qat_data = NULL;
    unsigned char *qat4k_data = NULL;
    ...
    // What IAXL_ZIP_JOB_SIZE relies on: a stateless job of at most 4 KB cannot emit a
    // back-reference past 4 KB, so even QAT output stays inside the window IAA decodes.
    rc = qat_zip_compress(0, input, IAA_HISTORY_LIMIT);
    if (rc == 0)
        rc = qat_zip_wait(0, &output, &output_len);
    if (rc != 0 || output_len <= 0) {
        fprintf(stderr, "[compat] 4 KB-job QAT compression failed: %d\n", rc);
        goto out;
    }
    qat4k_data = malloc((size_t)output_len);
    if (!qat4k_data)
        goto out;
    memcpy(qat4k_data, output, (size_t)output_len);
    int qat4k_len = output_len;

    rc = iaa_zip_decompress(0, qat4k_data, qat4k_len);
    if (rc != 0 || (rc = iaa_zip_wait(0, &output, &output_len)) != 0 ||
        verify("QAT compress (4 KB job) -> IAA decompress", input, IAA_HISTORY_LIMIT, output,
               output_len) != 0) {
        fprintf(stderr, "[compat] 4 KB-job QAT compress -> IAA decompress failed: %d\n", rc);
        failures++;
    }
    ...
    printf("[compat] note: IAA decodes at most a 4 KB history window, so QAT and CPU decompress "
           "anything while IAA only decodes IAA streams, 4 KB-window CPU streams and any stream "
           "produced by jobs of at most 4 KB (IAXL_ZIP_JOB_SIZE)\n");
    if (failures == 0)
        status = 0;
