...
static qpl_path_t g_path = qpl_path_hardware;

static qpl_path_t parse_path(const char *name) {
    if (!name || !*name)
        return qpl_path_hardware;
    if (!strcasecmp(name, "software") || !strcasecmp(name, "sw"))
        return qpl_path_software;
    if (!strcasecmp(name, "auto"))
        return qpl_path_auto;
    IAXL_CHECK(!strcasecmp(name, "hardware") || !strcasecmp(name, "hw"),
               "iaa_zip: IAXL_IAA_ZIP_PATH must be hardware, software or auto");
    return qpl_path_hardware;
}
...
// Collects the distinct NUMA nodes that own an IAA device. Spreading jobs over all
// of them is what makes throughput scale, so "auto" must not collapse to one node.
static int discover_numa_nodes(int *nodes, int max_nodes) {
    DIR *dir = opendir("/sys/bus/dsa/devices");
    if (!dir)
        return 0;

    int n = 0;
    for (struct dirent *ent = readdir(dir); ent && n < max_nodes; ent = readdir(dir)) {
    ...
    for (int i = 1; i < n; i++) {
        const int key = nodes[i];
        int j = i - 1;
        for (; j >= 0 && nodes[j] > key; j--)
            nodes[j + 1] = nodes[j];
        nodes[j + 1] = key;
    }
    return n;
...
// Parses the comma separated NUMA node list used to steer job submission.
// "auto" (the default) spreads instances over every NUMA node that owns an IAA device.
static int parse_numa_nodes(const char *env, int *nodes, int max_nodes) {
    if (!env || !*env || !strcasecmp(env, "auto")) {
        const int n = discover_numa_nodes(nodes, max_nodes);
        if (n > 0)
            return n;
        nodes[0] = -1;
        return 1;
...
static int slot_init(IaaSlot *sl, uint32_t job_size, int numa_id) {
    ...
    if (qpl_init_job(g_path, sl->job) != QPL_STS_OK)
        return -1;
...
int iaa_zip_init(void) {
    ...
    int instances_per_node = envs.IAXL_IAA_ZIP_INSTANCES_PER_DEVICE;
    ...
    g_path = parse_path(envs.IAXL_IAA_ZIP_PATH);
    ...
    const int node_count = parse_numa_nodes(dev_sel, nodes, MAX_NUMA_NODES);

    printf("[iaa_zip] config: path=%s numa_nodes=%s (%d resolved) instances/node=%d src_cap=%u B "
           "dst_cap=%u B queue_depth=%d\n",
           path_name(g_path), dev_sel ? dev_sel : "auto", node_count, instances_per_node, g_src_cap,
           g_dst_cap, g_queue_depth);

    uint32_t job_size = 0;
    IAXL_CHECK(qpl_get_job_size(g_path, &job_size) == QPL_STS_OK,
