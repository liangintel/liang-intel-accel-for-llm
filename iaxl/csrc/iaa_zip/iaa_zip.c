...
static int select_iaa_nodes(const char *env, IaaNode *out, int max_nodes) {
    ...
    if (!env || !*env || !strcasecmp(env, "auto")) {
        if (found_count > 0) {
            for (int i = 0; i < found_count && i < max_nodes; i++)
                out[i] = found[i];
            return found_count < max_nodes ? found_count : max_nodes;
        }
        out[0].node = -1;
        out[0].devices = 1;
        return 1;
    ...
    for (char *tok = strtok(buf, ","); tok && n < max_nodes; tok = strtok(NULL, ",")) {
        ...
        // Without sysfs we cannot count devices, so fall back to one instance per node.
        if (out[n].devices == 0) {
            IAXL_CHECK(found_count == 0, "iaa_zip: selected NUMA node owns no IAA device");
            out[n].devices = 1;
        }
        n++;
    }
...
int iaa_zip_init(void) {
    ...
    printf("[iaa_zip] config: path=%s numa_nodes=%s (%d node(s), %d device(s)) "
           "instances/device=%d src_cap=%u B dst_cap=%u B queue_depth=%d\n",
           path_name(g_path), dev_sel ? dev_sel : "auto", node_count, device_count,
           instances_per_device, g_src_cap, g_dst_cap, g_queue_depth);
